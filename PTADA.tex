...
\drawaline
\mymarginpar{\tiny{4. Lecture}} \vspace{-1pc}
...
\begin{definition}[Best $k$-rank approximation]Given $A\in \R^{m \times n}, A_k \in \R^{m\times n}$ is a \emph{best rank-$k$ approximation to $A$} with respect to a norm $\vvvert \cdot \vvvert$ if $A_k\in \underset{B\in \R^{m \times n}, \operatorname{rk} B\leq k}{\operatorname{argmin}}\vvvert A-B\vvvert$.
...
\drawaline
\mymarginpar{\tiny{5. Lecture}} \vspace{-1pc}
\begin{lemma}
